...
int VirtualTunerDaemonMain(int argc, char** argv) {
  ...
  flags.emplace_back(GflagsCompatFlag("pcm_server_address", pcm_server_address)
                         .Help("PCM streaming server UNIX socket address (e.g. "
                               "/tmp/virtual_tuner_pcm.sock)"));

  std::vector<std::string> args(argv + 1, argv + argc);
  auto parse_res =
      ConsumeFlags(flags, args, {.fail_on_unexpected_argument = false});
  ...
  if (pcm_server_address.empty()) {
    pcm_server_address = "/tmp/virtual_tuner_pcm.sock";
  }


  // PcmStreamServer takes a plain filesystem path. Strip any "unix:" scheme
  // prefix.
  if (pcm_server_address.rfind("unix:", 0) == 0) {
    pcm_server_address = pcm_server_address.substr(5);
  }
